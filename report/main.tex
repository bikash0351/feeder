...
\begin{itemize}
    ...
    \item \textbf{LSTM:} Processed sequential inputs with a 24-hour lookback window. Achieved moderate performance with RMSE of 0.498 and R\textsuperscript{2} of 0.014, demonstrating limited ability to capture complex temporal patterns compared to the explicitly engineered lag features used by MLP.
    \item \textbf{GRU:} Provided a computationally lighter alternative to LSTM with comparable performance, achieving RMSE of 0.499 and R\textsuperscript{2} of 0.012. Both recurrent architectures showed similar limitations in capturing the feeder's consumption dynamics.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Architecture-specific considerations for DL:} While the MLP achieved the best overall performance (R² = 0.915), recurrent architectures LSTM (R² = 0.014) and GRU (R² = 0.012) demonstrated comparatively limited predictive capability, reinforcing that deep learning architecture selection must be carefully matched to dataset characteristics and forecasting requirements.
